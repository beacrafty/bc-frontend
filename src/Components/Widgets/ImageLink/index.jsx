...
const ImageLink = ({ classes = {}, imgUrl, link, height, width, homeBanner = true, bgImg = false }) => {
    ...
    <>
      {imgUrl?.redirect_link?.link_type === "external_url" ? (
        <Link className="h-100" href={imgUrl?.redirect_link?.link || "/"} target="_blank">
          {bgImage ? <div className={`bg-size ${classes}`} style={{ backgroundImage: `url(${imgUrl?.image_url ? storageURL + imgUrl?.image_url : ""})`, borderRadius: "16px" }}></div> : <Image src={imgUrl?.image_url ? storageURL + imgUrl?.image_url : ""} className="bg-img w-100 img-fluid" alt="banner" height={height} width={width} />}
        </Link>
